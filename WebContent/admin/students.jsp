...
<%
    ...
    request.setAttribute("pageTitle", "Admin - Student Management");
%>
...
<section class="py-5 bg-light">
    <div class="container">
        <div class="d-flex flex-column flex-md-row justify-content-between align-items-md-center mb-4 gap-3">
            <div>
                <h3 class="fw-bold text-dark mb-1">Student & Job Seeker Directory</h3>
                <p class="text-muted mb-0">View registered candidates, block/unblock, or inspect detailed profiles & resumes</p>
            </div>
            <form action="${pageContext.request.contextPath}/admin/students" method="get" class="d-flex gap-2">
                <input type="text" name="search" class="form-control rounded-pill px-3" placeholder="Name or email..." value="<%= request.getAttribute("search") != null ? request.getAttribute("search") : "" %>">
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <% if (s.getSkills() != null) {
                                            for (int k = 0; k < Math.min(s.getSkills().size(), 3); k++) { %>
                                                <span class="badge bg-primary bg-opacity-10 text-primary border me-1"><%= s.getSkills().get(k) %></span>
                                        <%  }
                                           } %>
                                    </td>
                                    <td>
                                    ...
                                    <td class="text-end pe-4">
                                        <form action="${pageContext.request.contextPath}/admin/user-action" method="post" class="d-inline" onsubmit="return confirm('Block/Unblock this student?')">
                                            ...
                                            <button type="submit" class="btn btn-sm btn-outline-warning rounded-circle me-1" title="Block User"><i class="bi bi-slash-circle"></i></button>
                                        </form>
                                        <form action="${pageContext.request.contextPath}/admin/user-action" method="post" class="d-inline" onsubmit="return confirm('Permanently delete this candidate account?')">
                                            ...
                                            <button type="submit" class="btn btn-sm btn-outline-danger rounded-circle" title="Delete User"><i class="bi bi-trash"></i></button>
                                        </form>
                                    </td>
                                </tr>
                        <%  }
