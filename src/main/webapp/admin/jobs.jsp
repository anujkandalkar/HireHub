...
<%
    ...
    request.setAttribute("pageTitle", "Admin - Job Moderation");
%>
...
<section class="py-5 bg-light">
    <div class="container">
        <div class="d-flex flex-column flex-md-row justify-content-between align-items-md-center mb-4 gap-3">
            <div>
                <h3 class="fw-bold text-dark mb-1">System Job Moderation</h3>
                <p class="text-muted mb-0">Oversee all jobs posted across the platform and delete inappropriate listings</p>
            </div>
            <form action="${pageContext.request.contextPath}/admin/jobs" method="get" class="d-flex gap-2">
                <input type="text" name="search" class="form-control rounded-pill px-3" placeholder="Job title or company..." value="<%= request.getAttribute("search") != null ? request.getAttribute("search") : "" %>">
                    ...
                    <tbody>
                        ...
                        %>
                            <tr>
                                ...
                                <td><span class="badge bg-light text-dark border"><%= j.getApplicationCount() %></span></td>
                                ...
                                <td class="text-end pe-4">
                                    <div class="d-inline-flex gap-1">
                                        ...
                                        <form action="${pageContext.request.contextPath}/admin/job-action" method="post" class="d-inline" onsubmit="return confirm('Delete this job posting?')">
                                            <input type="hidden" name="jobId" value="<%= j.getId() %>">
                                            <input type="hidden" name="action" value="DELETE">
                                            <button type="submit" class="btn btn-sm btn-outline-danger rounded-circle" title="Delete Job"><i class="bi bi-trash"></i></button>
                                        </form>
                                    </div>
                                </td>
                            </tr>
